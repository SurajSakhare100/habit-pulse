    ...
    {week.map((day) => (
      ...
      <Tabs defaultValue="daily" className=" mx-auto my-6">
        <TabsList className="w-60  rounded-xl shadow-sm space-x-2 py-4">
          <TabsTrigger
            value="daily"
            className="w-full py-4  text-sm font-medium transition-all data-[state=active]:bg-primary data-[state=active]:text-white rounded-xl"
          >
            Daily
          </TabsTrigger>
          <TabsTrigger
        ...
        <TabsContent value="daily" className="w-full">
          {
            habits.map((habit) => (
              <DailyHabit habit={habit} today={todayDateStr} toggleDay={() => toggleDay(habit._id, todayDateStr)} />
            ))
          }
        </TabsContent>
